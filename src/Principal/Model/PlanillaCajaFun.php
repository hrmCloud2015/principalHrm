...
class PlanillaFunc extends AbstractTableGateway
{
   ...
   {
      $result=$this->adapter->query("select sum(valor) as valor , sueldo from ( 
        Select sum(e.devengado) + 
         ( case when ( (c.diasVaca > 0) and (c.diasVaca!=30) ) then (c.diasVaca * c.valorUniVaca) else 0 end ) as valor, c.sueldo  
                   ...
                   where g.idProc in (1,17) # procesos ley 100 y adicionales planila unica 
                        ...
                        and c.idEmp=".$idEmp." and a.id=".$id." 
union all
# Caso para vacaciones que toman el mes completo  
Select ( case when  c.diasVaca = 30 then (c.diasVaca * c.valorUniVaca) else 0 end ) as valor, c.sueldo  
                        from n_planilla_unica a 
                        inner join n_nomina b on year(b.fechaI) = a.ano and  month(b.fechaI) = a.mes # Traer datos del mes en cuestion
                        inner join n_planilla_unica_e c on c.idPla = a.id
                        inner join n_nomina_e d on d.idNom = b.id and d.idEmp = c.idEmp
                        ...
                        inner join n_tipemp j on j.id = i.idTemp 
                        left join n_vacaciones h on h.id = d.idVac 
                   where g.idProc in (1) and e.idConc in( 133 ) # procesos ley 100 y adicionales planila unica 
...
case when ( case f.tipo when 1 then sum(e.devengado)
      ...
      round( ( e.devengado / d.diasVac ), 2 ) as valorDiaVac, c.valorRetVaca # Vacciones por ingreso     
                        ...
                        inner join n_tipemp j on j.id = i.idTemp 
                        left join n_vacaciones h on h.id = d.idVac 
                        where ( g.idProc=17 or g.idProc=1 or j.tipo = 1  or j.tipo = 3 ) and e.idConc not in ('201','202','203','204','236','280',   '210','208',      '205','207' )   # 1 es proceso de ley 100  
      ...
      then (25 * ".$this->salarioMinimo.") 
         ...
         ( case f.tipo when 1 then (e.devengado)
              when 2 then (0) end )         
                        ...
                        inner join n_tipemp j on j.id = i.idTemp 
                        left join n_vacaciones h on h.id = d.idVac 
                        where ( g.idProc=1 or j.tipo = 1 ) and e.idConc not in ('201','202','203','204','236','280',   '210','208',      '205','207'  )   # 1 es proceso de ley 100  
...
//    then 0 else c.valorRetVaca end  )  )  ) as linea cambiada 
   ...
   public function getCaja($id, $idEmp)
   {
      ...
      inner join n_conceptos f on f.id = e.idConc
                        inner join n_conceptos_pr g on g.idConc = f.id
                        where g.idProc in (1,3) and e.idConc not in ('201','202','203','204','236','280', '205','207'  )   # 1 es proceso de ley 100  
                        and c.idEmp=".$idEmp." and a.id=".$id,Adapter::QUERY_MODE_EXECUTE);
      $datos = $result->current();      
      return $datos;
   }                
   ...
   public function getAus($id, $idEmp)
   {
      $result=$this->adapter->query("select sum( d.dias ) as diasAus 
                     ...
                     where a.id = ".$id." and f.luto = 0 and c.idEmp = ".$idEmp ,Adapter::QUERY_MODE_EXECUTE);
      $datos = $result->current();      
      return $datos;
   }                               
...
# Verificacion por aumento sin escala salarial 
        Select 
             case when b.salarioAct != b.salarioNue then 1 else 0 end as valor  
                        from n_asalarial a 
...
# Verificacion por aumento por escala salarial 
    select  case when b.salarioAct != b.salarioNue then 1 else 0 end as valor              
                        from n_asalarial a 
          ...
          case when ( month(d.fechaI)< a.mes and month(d.fechaF)>a.mes ) then # Disfurte de vacaciones dentro del periodo de planilla unica
            ...
            case when ( month(d.fechaI)< a.mes and month(d.fechaF)=a.mes ) then # Fin de vacaciones despeus de varios dias
                 ...
                 case when ( d.fechaF!=d.fechaR ) then # Cuando la fecha de regreso y fin de vacaciones son diferentes 
                    datediff( d.fechaR , concat( year(d.fechaF) ,'-', month(d.fechaF), '-01' ) )  
            else  
               ...
               datediff( d.fechaF , concat( year(b.fechaI) ,'-', month(b.fechaI), '-01' )  )  +1 
        end
          end 
     end      
end as diasVac , 
 ...
 ( select case when sum(ee.diasInc) is null then 0 else sum(ee.diasInc) end from n_nomina bb 
        ...
        d.valor / d.diasCal as valorVac             
           ...
           or ( (year( d.fechaI)=(a.ano-1) ) and ( (year(d.fechaF)=a.ano) and month(d.fechaF)=1 ) and d.idEmp = e.idEmp  ) 
                        where a.id = ".$id." and b.idTnom in (1,5) 
                          and c.idEmp = ".$idEmp."  and e.actVac = 0 group by d.id order by d.id desc limit 1" ,Adapter::QUERY_MODE_EXECUTE);
      $datos = $result->current();      
      return $datos;
   }
   ...
   {
      $result=$this->adapter->query("Select distinct c.contra, ( select case when sum( cc.dias ) > 30 then 30 else sum( cc.dias ) end as dias 
                    from n_nomina_e cc inner join n_nomina bb on bb.id = cc.idNom 
                      where cc.idEmp = c.idEmp and year(bb.fechaI) = year(b.fechaI) and month( bb.fechaI ) = month( b.fechaI )  ) as dias, d.finContrato    
                        from n_planilla_unica a 
                         inner join n_nomina b on year(b.fechaI) = a.ano and  month(b.fechaI) = a.mes 
...
rpad(  substring( rtrim(c.nit) , 1 ,length( rtrim(c.nit) ) - 1  ) , 16,' ') as nitEmpresa, '3E' as e, # Nit d
lpad( '' , 20,' ') as blanco20, 'U' as s, # para pamaretro
' ' as e01, 
lpad( '' , 9,' ') as blanco8, # pos 249 
'  ' as e02, # pos 259
 lpad( '' , 38,' ') as blanco38,
'14-23' as e1425, lpad( '' , 1,' ') as blanco1,   # NO SE QUE ES ESTO 
...
case when b.pensionado = 1 then '04' else '00' end as blanco2,  
 '  ' as e00, '47001' as ciuDepa , # configurar esto 
...
lpad( case when b.nRetiro =1 or b.at=1 then b.diasRiesgos else b.diasCaja end , 2,'0') as diasCaja,
lpad( b.sueldo , 9,'0') as salarioBase,
' ' as blan4 ,
lpad( b.ibcSalud , 9,'0') as ibcSalud, 
lpad( b.ibcPension , 9,'0') as ibcPension, 
lpad( b.ibcRiesgos , 9,'0') as ibcRiesgos,
lpad( b.ibcCaja , 9,'0') as ibcCaja,
lpad( round(b.porPension,4) , 6,'0') as porPension, # Aportes de pension
lpad( b.aporPension , 10,'0') as aporPension,
lpad( '' , 17,'0') as cerosPension,
lpad( round(b.aporPension,0) , 10,'0') as aporPension,
lpad( b.aporSol1 , 9,'0') as aporSolidaridad,
lpad( b.aporSol2 , 9,'0') as aporSolidaridad9,
...
, (SELECT @item := 0) item  
where a.id = ".$id."  and h.tipo in (0,1,3) 
order by lpad( @item := @item + 1, 5, '0' )" ,Adapter::QUERY_MODE_EXECUTE);
      $datos = $result->toArray();      
      return $datos;
   }             
