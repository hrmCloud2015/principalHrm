...
class PlanillaFunc extends AbstractTableGateway
{
   ...
   {
      $result=$this->adapter->query("Select sum(e.devengado) + 
...
case when ( case f.tipo when 1 then sum(e.devengado)
      ...
      round( ( e.devengado / d.diasVac ), 2 ) as valorDiaVac, c.valorRetVaca # Vacciones por ingreso     
                        ...
                        inner join n_tipemp j on j.id = i.idTemp 
                        left join n_vacaciones h on h.id = d.idVac 
                        where ( g.idProc=17 or g.idProc=1 or j.tipo = 1 ) and e.idConc not in ('201','202','203','204','236','280',   '210','208',      '205','207' )   # 1 es proceso de ley 100  
...
case when ( month(d.fechaI) = a.mes and month(d.fechaF) > a.mes ) then # Si inicia el periodo de vacaciones en el mes y termina en otro mes nomina actual tiene el idvac y dias 
 ...
 ( select case when sum(ee.diasInc) is null then 0 else sum(ee.diasInc) end from n_nomina bb 
   ...
   {
      $result=$this->adapter->query("Select distinct c.contra, ( select sum( cc.dias ) as dias 
                    from n_nomina_e cc inner join n_nomina bb on bb.id = cc.idNom 
                      where cc.idEmp = c.idEmp and year(bb.fechaI) = year(b.fechaI) and month( bb.fechaI ) = month( b.fechaI )  ) 
               
               + 
    
( select case when sum(cc.contra)>0 then sum( cc.diasI ) - 1  else 0 end 
                    from n_nomina_e cc inner join n_nomina bb on bb.id = cc.idNom 
                      where cc.idEmp = c.idEmp and year(bb.fechaI) = year(b.fechaI) and month( bb.fechaI ) = month( b.fechaI )  )                
               
               
               as dias, d.finContrato    
                        from n_planilla_unica a 
                         inner join n_nomina b on year(b.fechaI) = a.ano and  month(b.fechaI) = a.mes 
...
d.nit as nit,
rpad(  '890204162' , 16,' ') as nitEmpresa, d.e as e,
lpad( '' , 20,' ') as blanco20, d.s as s,
d.e01 as e01, lpad( '' , 8,' ') as blanco8, 
d.e02 as e02, lpad( '' , 38,' ') as blanco38,
d.e1425 as e1425, lpad( '' , 1,' ') as blanco1,   # NO SE QUE ES ESTO 
...
case when b.at = 1 then lpad(b.diasInc,2,'0') end as acct, # Accidente de trabajo  
...
, (SELECT @item := 0) item  
where a.id = ".$id." and b.codSuc = '".$idSuc."' and h.tipo in (0,1) 
order by lpad( @item := @item , 5, '0' )" ,Adapter::QUERY_MODE_EXECUTE);
      $datos = $result->toArray();      
      return $datos;
   }             
