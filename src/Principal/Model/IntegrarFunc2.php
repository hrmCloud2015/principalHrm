...
select h.id as idCcos, d.id as idFon, a.ano, a.mes, case when f.ter = 1 then d.nit else 0 end as nitCred , case when i.ter = 1 then d.nit else 0 end as nitDeb , d.nombre, 
...
( select sum( cc.deducido ) from n_nomina aa 
     ...
     b.aporSalud as aporPlanilla, 
                      ...
                      f.codigo as cuentaCred, g.codCtaD as cuentaDeb, 
     ...
     b.aporSalud as aporPlanilla, 
                 ...
                 case when i.ccos = 1 then h.codigo else 0 end as codCcosD ,'PENSION' as fondo, a.id , f.cxCxp as cxCxpC, i.cxCxp as cxCxpD     
                      from n_planilla_unica a 
                                ...
                                left join n_plan_cuentas f on f.codigo = e.codCta 
                                left join n_proviciones g on g.nombre = '6' # Salud
                                left join n_plan_cuentas i on i.codigo = g.codCtaD                                                               
                                left join n_cencostos h on h.id = c.idCcos 
                                where a.id =  ".$id." ) as a 
                     group by idCcos, idFon           
...
union all                              
# ARL
select h.id as idCcos, d.id as idFon,a.ano, a.mes, case when f.ter = 1 then d.nit else 0 end as nitCred ,
           case when i.ter = 1 then d.nit else 0 end as nitDeb , d.nombre, round(sum(b.aporRiesgos),0) as valor, 
...
# CAJA 
select h.id as idCcos, d.id as idFon,a.ano, a.mes, case when f.ter = 1 then d.nit else 0 end as nitCred,
 case when i.ter = 1 then d.nit else 0 end as nitDeb , d.nombre, round(sum(b.aporCaja),0) as valor, 
   0 as valorDescNom, 0 as aporPlanilla, 
    g.codCtaC as cuentaCred, g.codCtaD as cuentaDeb, case when i.ccos = 1 then h.codigo else 0 end as codCcosC, case when f.ccos = 1 then h.codigo else 0 end as codCcosD ,'CAJA' as fondo, a.id,         i.cxCxp as cxCxpC, f.cxCxp as cxCxpD , 0 as valorF   
                      from n_planilla_unica a 
                                inner join n_planilla_unica_e b on b.idPla = a.id 
                                inner join a_empleados c on c.id = b.idEmp 
...
union all                              
# SENA
select h.id as idCcos, d.id as idFon,a.ano, a.mes, case when f.ter = 1 then d.nit else 0 end as nitCred , case when i.ter = 1 then d.nit else 0 end as nitDeb  , 'SENA' as nombre, round(sum(b.aporSena),0) as valor,0 as valorDescNom, 0 as aporPlanilla,  g.codCtaC as cuentaCred, g.codCtaD as cuentaDeb, case when i.ccos = 1 then h.codigo else 0 end as codCcosC, case when f.ccos = 1 then h.codigo else 0 end as codCcosD ,'SENA' as fondo, a.id,        i.cxCxp as cxCxpC, f.cxCxp as cxCxpD, 0 as valorF                 
                      from n_planilla_unica a 
                                inner join n_planilla_unica_e b on b.idPla = a.id 
                                inner join a_empleados c on c.id = b.idEmp 
...
union all
# ICBF 
select h.id as idCcos, d.id as idFon,a.ano, a.mes, case when f.ter = 1 then d.nit else 0 end as nitCred , case when i.ter = 1 then d.nit else 0 end as nitDeb , 'ICBF' as nombre, round(sum(b.aporIcbf),0) as valor, 0 as valorDescNom, 0 as aporPlanilla, g.codCtaC as cuentaCred, g.codCtaD as cuentaDeb, case when i.ccos = 1 then h.codigo else 0 end as codCcosC, case when f.ccos = 1 then h.codigo else 0 end as codCcosD ,'ICBF' as fondo , a.id,         i.cxCxp as cxCxpC, f.cxCxp as cxCxpD, 0 as valorF    
                      from n_planilla_unica a 
                                inner join n_planilla_unica_e b on b.idPla = a.id 
                                inner join a_empleados c on c.id = b.idEmp 
   ...
   // Integracion planilla unica
   public function getIntegrarPlanillaVieja($id)
   {
      $result=$this->adapter->query("# FONDOS DE SALUD  
select a.ano, a.mes, case when f.ter = 1 then d.nit else 0 end as nitCred , case when i.ter = 1 then d.nit else 0 end as nitDeb , d.nombre, 
case when b.aprendiz = 1  then (b.aporSalud) else (sum(b.aporSalud)) - ( select sum( cc.deducido ) from n_nomina aa 
     ...
     where year(aa.fechaI) = a.ano and month(aa.fechaI) = a.mes and cc.idCcos = c.idCcos and cc.idFonS = b.idFonS and cc.idConc = 15  ) end as valor  , 
                      f.codigo as cuentaCred, g.codCtaD as cuentaDeb, case when f.ccos = 1 then h.codigo else 0 end as codCcosC, case when i.ccos = 1 then h.codigo else 0 end as codCcosD ,'SALUD' as fondo, a.id , f.cxCxp as cxCxpC, i.cxCxp as cxCxpD     
                      from n_planilla_unica a 
                                inner join n_planilla_unica_e b on b.idPla = a.id 
                                inner join a_empleados c on c.id = b.idEmp 
                                inner join t_fondos d on d.id = b.idFonS # ---------- fondo de salud 
...
( select sum( cc.deducido ) from n_nomina aa
     ...
     where year(aa.fechaI) = a.ano and month(aa.fechaI) = a.mes and cc.idCcos = c.idCcos  and cc.idFonP = b.idFonP and cc.idConc in (11,21)  )  end as valor,
       f.codigo as cuentaCred, g.codCtaD as cuentaDeb, case when f.ccos = 1 then h.codigo else 0 end as codCcosC, case when i.ccos = 1 then h.codigo else 0 end as codCcosD ,'PENSION' as fondo, a.id
            ,f.cxCxp as cxCxpC, i.cxCxp as cxCxpD      
                      from n_planilla_unica a 
                                inner join n_planilla_unica_e b on b.idPla = a.id 
                                inner join a_empleados c on c.id = b.idEmp 
                                inner join t_fondos d on d.id = b.idFonP # ---------- fondo de pension 
...
union all                              
# ARL
select a.ano, a.mes, case when f.ter = 1 then d.nit else 0 end as nitCred ,
           case when i.ter = 1 then d.nit else 0 end as nitDeb , d.nombre, round(sum(b.aporRiesgos),0) as valor, g.codCtaC as cuentaCred, 
           g.codCtaD as cuentaDeb, case when i.ccos = 1 then h.codigo else 0 end as codCcosC,
            case when f.ccos = 1 then h.codigo else 0 end as codCcosD ,'ARL' as fondo, a.id,        i.cxCxp as cxCxpC, f.cxCxp as cxCxpD   
                      from n_planilla_unica a 
                                inner join n_planilla_unica_e b on b.idPla = a.id 
                                inner join a_empleados c on c.id = b.idEmp 
                                inner join t_fondos d on d.id = b.idFonR  # riesgos 
...
# CAJA 
select a.ano, a.mes, case when f.ter = 1 then d.nit else 0 end as nitCred,
 case when i.ter = 1 then d.nit else 0 end as nitDeb , d.nombre, round(sum(b.aporCaja),0) as valor, g.codCtaC as cuentaCred, g.codCtaD as cuentaDeb, case when i.ccos = 1 then h.codigo else 0 end as codCcosC, case when f.ccos = 1 then h.codigo else 0 end as codCcosD ,'CAJA' as fondo, a.id,         i.cxCxp as cxCxpC, f.cxCxp as cxCxpD 
                      from n_planilla_unica a 
                                inner join n_planilla_unica_e b on b.idPla = a.id 
                                inner join a_empleados c on c.id = b.idEmp 
                                inner join t_fondos d on d.id = b.idCaja  
...
union all                              
# SENA
select a.ano, a.mes, case when f.ter = 1 then d.nit else 0 end as nitCred , case when i.ter = 1 then d.nit else 0 end as nitDeb  , 'SENA' as nombre, round(sum(b.aporSena),0) as valor, g.codCtaC as cuentaCred, g.codCtaD as cuentaDeb, case when i.ccos = 1 then h.codigo else 0 end as codCcosC, case when f.ccos = 1 then h.codigo else 0 end as codCcosD ,'SENA' as fondo, a.id,        i.cxCxp as cxCxpC, f.cxCxp as cxCxpD              
                      from n_planilla_unica a 
                                inner join n_planilla_unica_e b on b.idPla = a.id 
                                inner join a_empleados c on c.id = b.idEmp 
                                inner join t_fondos d on d.tipo = 7 # ---------- Sena
...
union all
# ICBF 
select a.ano, a.mes, case when f.ter = 1 then d.nit else 0 end as nitCred , case when i.ter = 1 then d.nit else 0 end as nitDeb , 'ICBF' as nombre, round(sum(b.aporIcbf),0) as valor, g.codCtaC as cuentaCred, g.codCtaD as cuentaDeb, case when i.ccos = 1 then h.codigo else 0 end as codCcosC, case when f.ccos = 1 then h.codigo else 0 end as codCcosD ,'ICBF' as fondo , a.id,         i.cxCxp as cxCxpC, f.cxCxp as cxCxpD 
                      from n_planilla_unica a 
                                inner join n_planilla_unica_e b on b.idPla = a.id 
                                inner join a_empleados c on c.id = b.idEmp 
                                inner join t_fondos d on d.tipo = 6 # ---------- Icbf 
...
# CAJA 
select a.ano, a.mes, case when f.ter = 1 then d.nit else 0 end as nitCred,
 case when i.ter = 1 then d.nit else 0 end as nitDeb , d.nombre, round(sum(b.aporCaja),0) as valor, g.codCtaC as cuentaCred, g.codCtaD as cuentaDeb, case when i.ccos = 1 then h.codigo else 0 end as codCcosC, case when f.ccos = 1 then h.codigo else 0 end as codCcosD ,'CAJA' as fondo, a.id            
                      from n_planilla_unica a 
                                inner join n_planilla_unica_e b on b.idPla = a.id 
                                inner join a_empleados c on c.id = b.idEmp 
                                inner join t_fondos d on d.id = b.idCaja  
...
union all                              
# SENA
select a.ano, a.mes, case when f.ter = 1 then d.nit else 0 end as nitCred , case when i.ter = 1 then d.nit else 0 end as nitDeb  , 'SENA' as nombre, round(sum(b.aporSena),0) as valor, g.codCtaC as cuentaCred, g.codCtaD as cuentaDeb, case when i.ccos = 1 then h.codigo else 0 end as codCcosC, case when f.ccos = 1 then h.codigo else 0 end as codCcosD ,'SENA' as fondo, a.id            
                      from n_planilla_unica a 
                                inner join n_planilla_unica_e b on b.idPla = a.id 
                                inner join a_empleados c on c.id = b.idEmp 
                                inner join t_fondos d on d.tipo = 7 # ---------- Sena
...
union all
# ICBF 
select a.ano, a.mes, case when f.ter = 1 then d.nit else 0 end as nitCred , case when i.ter = 1 then d.nit else 0 end as nitDeb , 'ICBF' as nombre, round(sum(b.aporIcbf),0) as valor, g.codCtaC as cuentaCred, g.codCtaD as cuentaDeb, case when i.ccos = 1 then h.codigo else 0 end as codCcosC, case when f.ccos = 1 then h.codigo else 0 end as codCcosD ,'ICBF' as fondo , a.id           
                      from n_planilla_unica a 
                                inner join n_planilla_unica_e b on b.idPla = a.id 
                                inner join a_empleados c on c.id = b.idEmp 
                                inner join t_fondos d on d.tipo = 6 # ---------- Icbf 
      ...
      $result=$this->adapter->query("select * from ( 
          ...
          concat( g.nombre , '- (', b.fechaI, '-' ,b.fechaF , ')' )  as detalle,  
              ...
              case when a.embargo>1 then a.nitEmb else  
                ...
                a.idCpres , a.idIinom, j.fecDoc, case when j.docRef!='' then j.docRef else 'SIN CODIGO' end as docRef, e.cxCxp, '' as empleado , '' as direccion                                                            
               from n_nomina_e_d_integrar a 
         ...
         select d.CedEmp as cedula, a.idInom,concat( year(now()) , '-', month(now())  , '-', day(now())   ) as fecha, b.fechaI, b.fechaF, a.codCta, 
               ...
               d.CedEmp as nit, 0 as idCcos, concat( g.nombre , '- (', b.fechaI, '-' ,b.fechaF , ')' )  as detalle,  
                 c.nombre as origen, case when d.formaPago = 1 
                  ...
                  0 as idCpres, 0 as idIinom, '' as fecDoc, '' as docRef, 0 as cxCxp,  concat( d.nombre , '' , d.apellido ) as empleado , d.DirEmp as direccion                                  
...
i.nombre as nomCta, a.nitTerD as nit, lpad( ltrim(c.codigo),4,'0') as idCcos, substring( ltrim(c.nombre), 1, 30) as nomCcos
, b.CedEmp, b.nombre as nomEmp, b.apellido, '  ' as error , e.nombre as nomGrupo       
   ...
   where d.estado = 2 and d.idTnom in (1,5) and d.id = ".$id."   
union all
